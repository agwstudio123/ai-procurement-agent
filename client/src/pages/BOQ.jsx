...
export default function BOQ() {
  ...
  async function placeOrder() {
    try {
      ...
      if (!contractor) {
        setMessage("❌ Contractor session not found. Login again.");
        return;
      }
      ...
      const response = await fetch(`${API_URL}/orders`, {
        ...
        body: JSON.stringify({
          ...
          amount: result.supplier.totalCost,

          totalAmount: result.supplier.totalCost,
        ...
        (result.success ? (
          <div className="bg-white rounded-xl shadow p-6 mt-8 max-w-xl">
            ...
            <p className="mt-3">
              ...
              <strong>
                {" "}
                {Number(result.supplier.totalCost).toLocaleString(
                  undefined,
                  {
            ...
            {result.marketPrice !== undefined && (
              <p className="mt-2">
                Market Price:
                <strong>
                  {" "}
                  {Number(result.marketPrice).toLocaleString(
                    undefined,
                    {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
            ...
            {result.savings !== undefined && (
              <p className="mt-2 text-green-600 font-semibold">
                Savings:
                {" "}
                {Number(result.savings).toLocaleString(
                  undefined,
                  {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
            ...
            <button
              onClick={placeOrder}
              className="mt-6 bg-green-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-green-700"
            >
              ✅ Place Order
            </button>
